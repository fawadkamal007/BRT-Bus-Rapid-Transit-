...
public class MainSceneController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
      ...
      stTableData.buildData(stTableView, "Select * from stationtable;");
            ticketData.buildData(sTicketTableView, "Select * from passengerTable,tickettable;");
     ...
     rs = QueryDatabase.query("Select Name from EmployeeTable where Designation='Employee';");
        if(rs!=null){
            try {
                while(rs.next()){
        ...
        empTableData.buildData(empTableView, "Select Eid,Name,Designation from employeeTable Order by(Eid);");
...
String stName = "";
    ...
    private void addBus(ActionEvent event) {
      ...
      boolean val = getBusFields();
       ...
       if(updateBus){
        ...
        busTableData.buildData(bTableView, "Select * from bustable Order By(BId);");
    ...
    private void aStation(ActionEvent event) {
        try {
            ...
            rSfields();
            stTableData.buildData(stTableView, "Select * from stationtable order By(Scode);");
    }
...
boolean updateBus = false;
    ...
    private void createTicket(ActionEvent event) {
        ...
        if(!passengerList.isEmpty()){
          ...
          resetCounterFields();  
          ticketData.buildData(sTicketTableView, "Select * from passengerTable,ticketable;");
   ...
   @FXML
    private void printTicket(ActionEvent event) {
       ...
       ObservableList<String> itemData = data.get(index);
       int id = Integer.parseInt(itemData.get(0));
       //retrieve data from ticket table for the above 
       
     ticketData.buildData(sTicketTableView, "Select * from tickettable;");
        //retrive all the columns from data structure
       GeneratePdf.pdf(bNum,tCSrc,tCDest,totalFare);
          }  
